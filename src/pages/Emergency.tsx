...
import { AlertTriangle, Droplets, Bug, Sun, ArrowRight } from "lucide-react";
import PageHeader from "@/components/PageHeader";

const emergencies = [
  { id: "flood", label: "🌊 Flood", icon: Droplets, color: "gradient-sky" },
  { id: "pest", label: "🐛 Pest Attack", icon: Bug, color: "gradient-warm" },
  { id: "drought", label: "☀️ Drought", icon: Sun, color: "gradient-danger" },
];
...
export default function Emergency() {
  ...
  return (
    <div className="pb-24 max-w-lg mx-auto">
      <PageHeader title="🚨 Emergency Mode" subtitle="Get instant survival steps" />

      <div className="px-4 space-y-4">
        <div className="bg-destructive/10 border border-destructive/20 rounded-2xl p-4 flex items-start gap-3">
          <AlertTriangle className="w-5 h-5 text-destructive mt-0.5 flex-shrink-0" />
          <p className="text-sm font-semibold text-foreground">Select your emergency for instant AI-guided survival steps</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {selected && (
            ...
            >
              <h3 className="font-bold text-base mb-3">⚡ Immediate Steps</h3>
