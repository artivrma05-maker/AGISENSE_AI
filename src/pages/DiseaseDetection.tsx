...
export default function DiseaseDetection() {
  ...
  const handleAnalyze = () => {
    setAnalyzing(true);
    setTimeout(() => { setResult(mockResult); setAnalyzing(false); }, 2000);
  };

  return (
    <div className="pb-24 max-w-lg mx-auto">
      <PageHeader title={t.diseaseDetectionTitle} subtitle={t.scanCropCamera} />
      <div className="px-4 space-y-4">
        {!image ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <p className="text-sm text-muted-foreground">{t.takePhotoAffected}</p>
            <div className="flex gap-2">
              <Button onClick={handleCapture} className="gradient-hero text-primary-foreground rounded-xl">
                <Camera className="w-4 h-4 mr-2" /> {t.capture}
              </Button>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            ...
            {!result && (
              <Button onClick={handleAnalyze} disabled={analyzing}
                ...
                {analyzing ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> {t.analyzingDots}</> : t.analyzeDisease}
              </Button>
            )}
          </motion.div>
        )}
        {result && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            <div className="bg-destructive/10 border border-destructive/20 rounded-2xl p-4">
              ...
              <p className="text-xs text-muted-foreground">{t.detectedOn}: {result.crop}</p>
            </div>
            <div className="bg-card rounded-2xl p-4 shadow-card">
              <h3 className="font-bold text-sm flex items-center gap-1.5 mb-2">
                <Leaf className="w-4 h-4 text-primary" /> {t.remedy}
              </h3>
              <p className="text-sm text-muted-foreground leading-relaxed">{result.remedy}</p>
            </div>
            <div className="bg-card rounded-2xl p-4 shadow-card">
              <h3 className="font-bold text-sm mb-2">🛡️ {t.prevention}</h3>
              <p className="text-sm text-muted-foreground leading-relaxed">{result.prevention}</p>
            </div>
            <Button variant="outline" onClick={() => { setImage(null); setResult(null); }} className="w-full rounded-xl">
              {t.scanAnotherPlant}
            </Button>
          </motion.div>
        )}
